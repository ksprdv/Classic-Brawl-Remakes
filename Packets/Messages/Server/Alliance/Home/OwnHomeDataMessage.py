...
class OwnHomeDataMessage(Writer):
    ...
    def encode(self):
        ...
        for skin_id in self.player.UnlockedSkins: # skin_id
        ...
        self.writeVint(60) # Change Name Cost in Gems
        ...
        self.writeVint(self.player.battle_tokens) # Battle tokens remaining
        ...
        self.writeVint(len(Shop.tickets))
        for item in Shop.tickets:  # Tickets price
            self.writeVint(item['Cost'])

        self.writeVint(len(Shop.tickets))
        for item in Shop.tickets:  # Tickets amount
